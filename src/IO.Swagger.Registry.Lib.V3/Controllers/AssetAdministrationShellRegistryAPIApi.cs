...
public class AssetAdministrationShellRegistryAPIApiController : ControllerBase
{
    ...
    public AssetAdministrationShellRegistryAPIApiController(IAppLogger<AssetAdministrationShellRegistryAPIApiController> logger, IBase64UrlDecoderService decoderService,
                                                            ...
                                                            IAasDescriptorPaginationService paginationService, IAASUnitOfWork unitOfWork)
    {
        _logger                     = logger;
                                                                           ...
                                                                           [FromQuery] string? assetType)
    {
        ...
        else
        {
            ...
            var lstAAS = _unitOfWork.AASSets.AsFetchable().Where(x => assetList.Count == 0 || assetList.Contains(x.GlobalAssetId))
                .Select(x => _aasRegistryService.CreateAasDescriptorFromDB(x));

            aasDescriptors.AddRange(lstAAS);
        }
    ...
    [SwaggerResponse(statusCode: 500, type: typeof(Result), description: "Internal Server Error")]
    [SwaggerResponse(statusCode: 0, type: typeof(Result), description: "Default error handling for unmentioned status codes")]
    public virtual async Task<IActionResult> GetAssetAdministrationShellDescriptorById([FromRoute] [Required] string aasIdentifier)
    {
        ...
        {
            // from database
            var assets = await _unitOfWork.AASSets.AsFetchable().ToListAsync();
            foreach (var aasDB in assets)
            {
                if (decodedAasIdentifier == null || !decodedAasIdentifier.Equals(aasDB.Identifier, StringComparison.Ordinal))
                {
                    continue;
                }
        ...
        [FromQuery] string? assetId)
    {
        try
        {
            ...
            else
            {
                ...
                var lstAAS = _unitOfWork.AASSets.AsFetchable().Where(x => withoutAssetId || assetList.Contains(x.GlobalAssetId))
                    .Select(x => x.Identifier);

                aasList.AddRange(lstAAS);
            }
